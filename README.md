...
- **`assets/`** — 公共资源：`tokens.css`（全站设计变量，含暗色模式）、`tool-page.css`（工具页公共样式）、`clipboard.js`（复制封装）、`registry.js`（工具注册表）、`palette.js`（Ctrl+K 命令面板）、`vendor/`（第三方库本地副本）。
...
3. 在 **`assets/registry.js`** 注册表中补一条（命令面板与站内搜索依赖它）；
